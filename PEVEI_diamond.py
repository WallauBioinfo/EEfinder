...
parser.add_argument("-db1", '--databasefilter',
                    help="Host and heatshock proteins database file.")
parser.add_argument("-db2", "--databaseTE", help="Transposon database file.")
parser.add_argument("-bt", "--blasttype", help="Choose the blast strategy against tranposons database, default = blastn.",
                    default='blastn', choices=['blastn', 'blastx', 'tblastx'])
parser.add_argument(
...
def makediamonddb(data, threads):
    ...
    cmd_clinedb.wait()
    return(print(f'Makediamonddb for {data}: DONE'))


def rundiamond(query_file, database_file, threads):
    """
    ...
    clinedmd = f"diamond blastx -p {threads} -d {database_file}.dmnd -f 6 -q {query_file} -o {query_file}.blastx -e 0.00001 --matrix BLOSUM45 -k 500 --max-hsps 0 --very-sensitive"
    clinedmd = shlex.split(clinedmd)
    cmd_clinedmd = subprocess.Popen(clinedmd)
    cmd_clinedmd.wait()
    return(print(f'DIAMOND with {query_file} against {database_file}: DONE'))
...
def rundiamonddb1(query_file, database_file, threads):  # colocar opções de blastn ou blastx
    """
    ...
    clinedmddb1 = f"diamond blastx -p {threads} -d {database_file}.dmnd -f 6 -q {query_file} -o {query_file}.blastx -e 0.00001 --matrix BLOSUM45 -k 500 --max-hsps 0 --very-sensitive"
    clinedmddb1 = shlex.split(clinedmddb1)
    cmd_clinedmddb1 = subprocess.Popen(clinedmddb1)
    cmd_clinedmddb1.wait()
    return(print(f'DIAMOND with {query_file} against {database_file}: DONE'))
...
def compare_blasts(vir_blast_filtred, filter_blast_filtred):
    ...
    df_hybrid = df_hybrid.sort_values(
        by=['qseqid', 'bitscore'], ascending=False)
    ...
    df_nr_vir.to_csv(filter_blast_filtred+'.concat.nr.vir',
                     sep='\t', index=False)
    return(print("DONE"))


def rundiamonddb2(query_file, database_file, threads):
    """
    ...
    clinedmddb2 = f"diamond blastx -p {threads} -d {database_file}.dmnd -f 6 -q {query_file} -o {query_file}.blastx -e 0.00001 --matrix BLOSUM45 -k 500 --max-hsps 0 --very-sensitive"
    clinedmddb2 = shlex.split(clinedmddb2)
    cmd_clinedmddb2 = subprocess.Popen(clinedmddb2)
    cmd_clinedmddb2.wait()
    return(print(f'DIAMOND with {query_file} against {database_file}: DONE'))
...
if __name__ == '__main__':
    ...
    if step == 'All' or step == 'EVEfinder':
        ...
        get_fasta(prefix+".rn.fmt", prefix+'.rn.fmt.blastx.filtred.bed',
                  prefix+'.rn.fmt.blastx.filtred.bed.fasta')
        makediamonddb(database_filter, threads)
        rundiamonddb1(prefix+'.rn.fmt.blastx.filtred.bed.fasta',
                      database_filter, threads)
        blastx_filter(
            prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx', "HOST")
        compare_blasts(prefix+'.rn.fmt.blastx.filtred', prefix +
                       '.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred')
    ...
    if step == 'All' or step == 'Tax':
        ...
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir', 'r') as putative_EVEs, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tmp', 'w+', newline='') as tax_out_tmp:
            ...
            for var_prot_id in prot_id:
                try:
                    handle_code = Entrez.efetch(
                        db='protein', id=var_prot_id, rettype='gb')
                    time.sleep(0.15)
                    handle_code2 = Entrez.efetch(
                        db='protein', id=var_prot_id, rettype='gb')
                    handle_prot_var = str(re.findall(
                        r'\/product=.*', handle_code.read()))
                    handle_code_var = str(re.findall(
                        r'\/db_xref="taxon:.*', handle_code2.read()))
                    handle_code_var = re.sub(r"\[.*:", '', handle_code_var)
                    handle_code_var = re.sub(r'".*\]', '', handle_code_var)
                    handle_prot_var = re.sub(r'.*=', '', handle_prot_var)
                    handle_prot_var = re.sub(r"'\]", '', handle_prot_var)
                    handle_prot_var = re.sub(r'"', '', handle_prot_var)
                    tax_reference_list.append(
                        [var_prot_id, handle_code_var, handle_prot_var])
            ...
            for i in tax_reference_list:
                ...
                try:
                    ...
                    continue
                tax_levels_list.append(
                    [prot_id, prot_prod, tax_id, sk, od, fm, gn, sp])
            ...
            for eve_line in eve_infos_reader:
                if "qseqid" in eve_line:
                    ...
                    with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tmp', 'r') as tax_out_tmp:
                        tax_tmp_reader = csv.reader(
                            tax_out_tmp, delimiter='\t')
        ...
        df_tax.to_csv(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax',
                      sep='\t', index=False, na_rep='Unclassified')
        df_to_bed = df_tax[['Element-ID', 'Order', 'Family',
                            "Qstart", 'Qend', 'Protein-ID', 'Sense']]
        df_to_bed = df_to_bed.sort_values(
            by=['Element-ID', 'Order', 'Family', 'Qstart'])
        df_to_bed.to_csv(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.tmp',
                         sep='\t', index=False, header=False, na_rep='Unclassified')
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.tmp', 'r') as tmp_to_bed:
            tmp_header = ['Element', 'Order', 'Family',
                          'Start', 'End', 'Protein', 'Sense']
            df = pd.read_csv(tmp_to_bed, sep='\t',
                             header=None, names=tmp_header)
            df['Element'] = df['Element'].str.replace(r":.*", "")
            names_var = df[['Element', 'Order', 'Family', 'Sense']].agg(
                '|'.join, axis=1)
            ...
            df_2 = df_2.sort_values(
                ['Bed_name', 'Start'], ascending=(True, True))
            df_2.to_csv(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed',
        ...
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge', "w") as bed_merge_out:  # abm = annotation bed merged
            bed_merge_cmd = 'bedtools merge -d '+limit_merge+' -i '+prefix + \
                '.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed' + \
                ' -c 4 -o collapse -delim "AND"'
            bed_merge_cmd = shlex.split(bed_merge_cmd)
            bed_merge_process = subprocess.Popen(
                bed_merge_cmd, stdout=bed_merge_out)
        ...
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt', "w") as bed_merge_fmt:
            remove_annot = "sed -e 's/|.*ae|pos//g' -e 's/|.*ied|pos//g' -e 's/|.*ae|neg//g' -e 's/|.*ied|neg//g' "+prefix + \
                '.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge'  # format bed_name
            # Split the string using shell-like syntax.
            remove_annot = shlex.split(remove_annot)
            cmd_remove_annot = subprocess.Popen(
                remove_annot, stdout=bed_merge_fmt)
            cmd_remove_annot.wait()
        get_fasta(prefix+".rn.fmt", prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt',
                  prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta')
        ...
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt', 'r') as bed_merge_file, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.tax', 'w') as bed_merge_tax_out:
            ...
            bed_merge_tax_out_writer = csv.writer(
                bed_merge_tax_out, delimiter='\t')
            bed_merge_tax_out_writer.writerow(
                ["Element-ID", "Protein-IDs", "ProteinProducts", "Order", "Family", "Genus", "Species"])
            count = 0
            for line in bed_merge_file_reader:
                ...
                bed_merge_tax_list.append(
                    [element_merged_id, protein_ids, protein_terms, vir_order, vir_family, vir_genus, vir_specie])
    ...
    if step == 'All' or step == 'TEs':
        ...
        if database_TE != None:
            transposon_terms = ["recombinase", "Recombinase", "transposase", "Transposase",
                                "Helicase", "helicase", "LINE", "gag-like", "RNA-dependent DNA polymerase", "repeat"]
            putative_transposon_tax = ["Ortervirales", "Phycodnaviridae", "Polydnaviridae", "Mimiviridae", "Unclassified",
                                       "Iridoviridae", "Caudovirales", "Poxviridae", "Baculoviridae", "Herpesviridae", "Marseilleviridae"]
            ...
            if blast_type == 'blastn' or blast_type == 'tblastx':
                makediamonddb(database_TE, 'nucl')
            elif blast_type == 'blastx':
                makediamonddb(database_TE, 'prot')
            rundiamonddb2(
                prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.pTEs', database_TE, threads)
            with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta', 'r') as EVEs_merged_seq, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.pTEs.blastx', 'r') as blast_TEs_out:  # creates a read file for blast output
                ...
                for line in lines:  # read each line of query file
                    if ">" in line:  # for each header in query file
                        ...
                        # if header of query file exist in blast output, nothing occours
                        if (all_blast_TEs_lines.find(var_seq_id)) != -1:
                            pass
            ...
            with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta', 'r') as EVEs_merged_seq, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta', 'w') as output_seq:
                # creates a dictionary with sequences of query file
                record_dict = SeqIO.to_dict(
                    SeqIO.parse(EVEs_merged_seq, 'fasta'))
                # search for no_match sequences in query file
                result_records = [record_dict[id_] for id_ in no_match_TE]
                # write an output file with queries that don't present match in previous analysis
                SeqIO.write(result_records, output_seq, "fasta")
        ...
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.tax', 'r') as tax_file, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.tax', 'w') as with_repeat_eves_tax_out, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.cl.tax', 'w') as without_repeat_eves_tax_out:
            csv_reader = csv.reader(tax_file, delimiter='\t')
            writer_with_repeat_eves_tax_out = csv.writer(
                with_repeat_eves_tax_out, delimiter='\t')
            writer_without_repeat_eves_tax_out = csv.writer(
                without_repeat_eves_tax_out, delimiter='\t')
            with_repeat_rows = []
            without_repeat_rows = []
            for line in csv_reader:
            ...
            writer_with_repeat_eves_tax_out.writerow(
                ["Element-ID", "Protein-IDs", "ProteinProducts", "Order", "Family", "Genus", "Species"])
            writer_without_repeat_eves_tax_out.writerow(
                ["Element-ID", "Protein-IDs", "ProteinProducts", "Order", "Family", "Genus", "Species"])
    ...
    if step == 'All' or step == 'Flank':
        with open(prefix+'.rn.fmt.length', 'w') as output_length:
            ...
            for seq_record in SeqIO.parse(prefix+'.rn.fmt', "fasta"):
                output_length.write(seq_record.id+'\t' +
                                    str(len(seq_record))+'\n')
        ...
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta', 'r') as with_repeat_eves, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.cl', 'r') as without_repeat_eves, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.bed', 'w') as with_repeat_eves_bed_out, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.cl.bed', 'w') as without_repeat_eves_bed_out:
            ...
            for line in with_repeat_eves_lines:
                if '>' in line:
                    ...
                    with_repeat_eves_bed_out.write(
            ...
            for line in without_repeat_eves_lines:
                if '>' in line:
                    ...
                    line_start = re.sub('-.*', '', line_start).rstrip('\n')
                    line_end = re.sub('.*-', '', line).rstrip('\n')
                    without_repeat_eves_bed_out.write(
                        line_name+'\t'+line_start+'\t'+line_end+'\n')
        ...
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl.bed', "w") as flank_out:
            bed_flank_cmd = 'bedtools flank -i '+prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.cl.bed' + \
                ' -g '+prefix+'.rn.fmt.length'+' -b '+str(flank_region)
            bed_flank_cmd = shlex.split(bed_flank_cmd)
            bed_flank_process = subprocess.Popen(
                bed_flank_cmd, stdout=flank_out)
        ...
        get_fasta(prefix+'.rn.fmt', prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl.bed.fmt',
                  prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl.fasta')
        get_fasta(prefix+'.rn.fmt', prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl-left.bed.fmt',
                  prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl.left.fasta')
        get_fasta(prefix+'.rn.fmt', prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl-right.bed.fmt',
                  prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl.right.fasta')
        ...
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.fl.bed', "w") as flank_out:
            bed_flank_cmd = 'bedtools flank -i '+prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.bed' + \
                ' -g '+prefix+'.rn.fmt.length'+' -b '+str(flank_region)
            bed_flank_cmd = shlex.split(bed_flank_cmd)
            bed_flank_process = subprocess.Popen(
                bed_flank_cmd, stdout=flank_out)
        ...
        with open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.fl.bed', 'r+') as flank_bed, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.fl-left.bed.fmt', 'w') as flank_left_bed_fmt, open(prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.fl-right.bed.fmt', 'w') as flank_right_bed_fmt:
            ...
            for line in reader:
                ...
                try:
                    ...
                    pass
        get_fasta(prefix+'.rn.fmt', prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.fl.bed.fmt',
                  prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fl.fasta')
        get_fasta(prefix+'.rn.fmt', prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.fl-left.bed.fmt',
                  prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fl.left.fasta')
        get_fasta(prefix+'.rn.fmt', prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fasta.fl-right.bed.fmt',
                  prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fl.right.fasta')

    ###############################>FLANKING-BLASTs<###############################
    if step == 'All' or step == 'Flank-blast':
        rundiamonddb2(
            prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fl.left.fasta', database_TE, threads)
        rundiamonddb2(
            prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fl.right.fasta', database_TE, threads)
        rundiamonddb2(
            prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl.left.fasta', database_TE, threads)
        rundiamonddb2(
            prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl.right.fasta', database_TE, threads)
        blastx_filter(
            prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fl.left.fasta.blastx', 'HOST')
        blastx_filter(
            prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.fl.right.fasta.blastx', 'HOST')
        blastx_filter(
            prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl.left.fasta.blastx', 'HOST')
        blastx_filter(
            prefix+'.rn.fmt.blastx.filtred.bed.fasta.db_filter.blastx.filtred.concat.nr.vir.tax.bed.merge.fmt.fasta.noTEs.cl.fl.right.fasta.blastx', 'HOST')
