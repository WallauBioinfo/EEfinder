...
parser = argparse.ArgumentParser(
    description="This script receives a fasta file and execute a getorf analysis and remove redundant ORFs from getorf output based on clusterization analysis",
...
parser.add_argument(
    "-ms",
    "--minsize",
    help="minsize getorf argument 'Minimum nucleotide size of ORF to report.'",
    default="100",
    type=str,
)
parser.add_argument(
    "-p", "--threads", help="Threads for cd-hit-est analyze.", default=1, type=str
)
parser.add_argument(
    "-m", "--memory", help="Memory in MB for cd-hit-est analyze", default=2000, type=str
)
...
getorf_csv_writer.writerow(
    ["ORF-ID", "Element-ID", "ORF", "ORF-Start", "ORF-End", "ORF-Length(aa)", "Sense"]
)
getorf_info_list = []
    ...
    for line in getorf_reader:
        if ">" in line:
            ...
            if "REVERSE" in ORF_id:
                ...
                sense = "neg"
                getorf_info_list.append(
                    [ORF_id, Element_id, ORF, ORF_end, ORF_start, ORF_length, sense]
                )
            else:
                ORF_length = (int(ORF_end) - int(ORF_start) + 1) / 3
                sense = "pos"
                getorf_info_list.append(
                    [ORF_id, Element_id, ORF, ORF_start, ORF_end, ORF_length, sense]
                )
