...
parser = argparse.ArgumentParser(
    description="This script retrieve a formatted table for using on EEfinder",
    formatter_class=argparse.RawTextHelpFormatter,
)

parser.add_argument("-in", "--input", help="File with access IDs", required=True)
parser.add_argument("-em", "--email", help="Email for using on Entrez", required=True)
parser.add_argument(
    "-key", "--APIkey", help="Key for not limiting number attemps", required=True
)
...
def efetch_function(data):
    ...
    handle = Entrez.efetch(db="protein", id=data, retmode="xml", rettype="gb")
    return handle.read()
...
if __name__ == "__main__":
    ...
    with open(f"{sequences}.meta", "w", newline="") as output:
        ...
        for accession in ids_list:
            ...
            taxid = re.sub(r"<.*\]", "", taxid)
            handle_tax_data = Entrez.efetch(db="Taxonomy", id=taxid, retmode="xml")
            ...
            try:
                ...
                print(f"done organism taxonomy for {accession}:{taxid}", end="\n")
            except:
                print(
