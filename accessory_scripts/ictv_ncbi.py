...
parser = argparse.ArgumentParser(
    description="This script a csv file and returns protein information by viral family."
)
...
for species in df.Species:
    ...
    try:
        with open(specie_name + ".prot.fasta", "a") as output_specie:
            prids = get_ids(
                Entrez.esearch(
                    db="Protein", term=f"{species}[Scientific Name]", retmode="json"
                )
            )
            for prid in prids:
                fasta = Entrez.efetch(
                    db="Protein", id=prid, rettype="fasta", retmode="text"
                ).read()
        ...
        subprocess.call(
            [f"sed -i -e 's/\[.*\]/{species}/g' {specie_name}.prot.fasta.cd"],
            shell=True,
        )
