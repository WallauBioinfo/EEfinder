...
parser.add_argument(
    "-in", "--input", help="Fasta file, e.g. example.fasta", required=True
)
parser.add_argument(
...
with open(input_file + ".cd.clstr", "r") as cluster_file, open(
    input_file + ".cd.clstr.tsv", "w"
) as cluster_formated:
    cluster_file_reader = cluster_file.readlines()
    cluster_formated_writer = csv.writer(cluster_formated, delimiter="\t")
    cluster_formated_writer.writerow(
        ["Sequence", "Assembly-Code", "Specie-Code", "Cluster", "Representative"]
    )
    cluster_list = []
