...
def insert_prefix(input_file, prefix):
    """
    ...
    """
    with open(input_file,'r') as input_file_reader, open(f"{input_file}.rn",'w+') as output_file:
...
def concat_files(file1,file2,outdir,output):
    ...
    print(f"DONE: The concatenated filter database is stored at {outdir}{output}\n")

class SetPrefix():
    def __init__(self, in_file, prefix):
        self.in_file = in_file
        self.prefix = prefix
        
    def run_insert_prefix(self):
        insert_prefix(self.in_file, self.prefix)
