...
###############################>ARGUMENTS<###############################
parser = argparse.ArgumentParser(description = 'Create stacked barplots from flaking regions blast results')
parser.add_argument("-in", "--input", help=".Tax file generated by PEVEI.py", required=True)
parser.add_argument("-bf","--blastfiltred",help=".noTEs.fl.left.fasta.rn.blast.filtred file generated by PEVEI.py", required=True)
...
parser.add_argument("-ste","--specifictes",help="Pass specific TEs families to generate plots, e.g. Gypsy, Bel-Pao",nargs='+')
parser.add_argument("-gt","--groupbytax",help="Group families in others category by specific treshold of element numbers, default = 5", default=5, type=int)
...
tsv_merged_writer.writerow(["Element-ID","Length","ProteinProducts","Super-Kingdom","Order","Family","Genus","Species","Sense","TE-blast","Pident","Length","Evalue","Bitscore"])
info_list = []
        ...
        with open(blast_file,'r') as blast_file_tsv:
            ...
            for line_blast in blast_file_tsv_reader:
                if line_tax[0] in line_blast[0]:
                    info_list.append([line_tax[0],line_tax[1],line_tax[3],line_tax[4],line_tax[5],line_tax[6],line_tax[7],line_tax[8],line_tax[9],
                    line_blast[1],float(line_blast[2]),float(line_blast[3]),float(line_blast[10]),float(line_blast[11])])
...
with open(blast_file+'.info','r') as info_tsv_file:
    ...
    df = df.drop_duplicates(subset=['Element-ID']).sort_values(by=['TE-blast','Bitscore'],ascending = False)
    ...
    if specific_tax == None:
        ...
        barplot.legend(loc='upper center', ncol=6, fancybox=True, prop={'size': 12},title='TEs', labels=labels, title_fontsize=10)
    ...
    if specific_tax != None:
        ...
        sns.despine(fig=None, top=True, right=True, left=True, bottom=False, offset=None, trim=False)
        barplot.legend(loc='upper center', ncol=6, fancybox=True, bbox_to_anchor=(0.45, 1.20), prop={'size': 12},title='TEs', labels=labels, title_fontsize=10)
        plt.xlabel('Count', fontsize=10)
        plt.ylabel('')
        plt.xticks(fontsize=10)
        plt.yticks(fontsize=8)
