...
parser.add_argument("-p","--threads",help="Threads for Mafft analysis", default = 1, type=str)
...
with open(input_file+'.algn','w') as mafft_out:
    mafft_cmd = F'mafft --thread '+str(var_threads)+' --reorder --adjustdirection --bl 45 --auto '+input_file
...
CIAling_matrix = subprocess.call([F"CIAlign --infile {input_file}.algn --outfile_stem {input_file}.matrix1 --make_similarity_matrix_input"], shell=True)
...
CIAling_matrix = subprocess.call([F"CIAlign --infile {input_file}.algn --outfile_stem {input_file}.01divergent --remove_divergent --remove_divergent_minperc {average_similarity} --plot_markup"], shell=True)
#Remove Inserions
CIAling_matrix = subprocess.call([F"CIAlign --infile {input_file}.01divergent_cleaned.fasta --outfile_stem {input_file}.02insertions --remove_insertions --plot_markup"], shell=True)
#Crop ends
CIAling_matrix = subprocess.call([F"CIAlign --infile {input_file}.02insertions_cleaned.fasta --outfile_stem {input_file}.03ends --crop_ends --plot_markup"], shell=True)
#Make a 2nd distance matrix
CIAling_matrix = subprocess.call([F"CIAlign --infile {input_file}.03ends_cleaned.fasta --outfile_stem {input_file}.matrix2 --make_similarity_matrix_input"], shell=True)
...
CIAling_matrix = subprocess.call([F"CIAlign --infile {input_file}.03ends_cleaned.fasta --outfile_stem {input_file}.04divergent2 --remove_divergent --remove_divergent_minperc {average_similarity} --plot_markup"], shell=True)
#Organising results
mv_output = subprocess.call([F"mv {input_file}.04divergent2_cleaned.fasta {input_file}.algn.edited"], shell=True)
move_results = subprocess.call([F"mv *divergent* *matrix* *insertions* *ends* {dir_name}"], shell=True)